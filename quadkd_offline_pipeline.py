...
TEACHER_ORDER = ["frame_dfdc", "frame_diff", "video_ff", "audio_lavdf"]
TEACHER_BRANCH = {
    "frame_dfdc": "frame",
    "frame_diff": "frame",
    "video_ff": "temporal",
...
# ---- run control ----
OUT_DIR = Path("/kaggle/working/kd_final")
CKPT_DIR = OUT_DIR / "checkpoints"
RESUME_CKPT_PATH = "/kaggle/input/models/syedazmulhasansabbir/last-checkpoint-2/tensorflow2/default/1/last_checkpoint.pth"
FORCE_RESTART_CURRENT_EPOCH = True
BEST_CKPT_RESUME_INPUT_PATH = "/kaggle/input/models/syedazmulhasansabbir/best-ema-auc-pth/tensorflow2/default/1/best_ema_auc.pth"
...
# a PRECOMPUTE_TEACHERS session, save /kaggle/working/kd_final/teacher_artifacts
# as a dataset, then point this at the mounted path next session). Leave "" for
# a brand-new run with no artifacts to resume from yet.
ARTIFACT_RESUME_INPUT_DIR = "/kaggle/input/notebooks/syedazmulhasan2001/ultimate-model-5/kd_final/teacher_artifacts"
...
if TORCH_OK:
    ...
    def _maybe_restore_best_checkpoint_from_input():
        ...
        best_path = CKPT_DIR / "best_auc.pth"
        if best_path.exists():
            return  # already present this session
        if not BEST_CKPT_RESUME_INPUT_PATH:
            print("[checkpoint] BEST_CKPT_RESUME_INPUT_PATH is empty; expecting "
                  "best_auc.pth to already be present under kd_final/checkpoints/, "
                  "or EVALUATE will fail.")
            return
        if not Path(BEST_CKPT_RESUME_INPUT_PATH).exists():
            print(f"[checkpoint] BEST_CKPT_RESUME_INPUT_PATH={BEST_CKPT_RESUME_INPUT_PATH} "
                  f"does not exist; EVALUATE will fail if no checkpoint is found.")
            return
        import shutil
        resolved = resolve_ckpt_file(BEST_CKPT_RESUME_INPUT_PATH)
        CKPT_DIR.mkdir(parents=True, exist_ok=True)
        shutil.copy2(resolved, best_path)
        print(f"[checkpoint] restored best_auc.pth from {BEST_CKPT_RESUME_INPUT_PATH} "
              f"(resolved to {resolved}) -> {best_path}")
...
if TORCH_OK:
    ...
    def run_comprehensive_evaluation_from_artifacts(student: "nn.Module",
        ...
        pd_safe_write_csv([dict(manipulation=k, **v) for k, v in per_manip.items()], CSV_DIR / "error_analysis.csv")
    ...
    def main():
        ...
        if MODE == "EVALUATE":
            ...
            if not best_path.exists():
                raise RuntimeError(f"[main] {best_path} not found; run MODE='TRAIN_STUDENT' to "
                                    f"completion (or at least one improving epoch) first.")
