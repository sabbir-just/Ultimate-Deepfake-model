...
================================================================================
 ...
 REDESIGNED — Offline multi-teacher knowledge distillation into a compact
...
# decord has its OWN native logger (dmlc-core, glog-style), separate from ffmpeg's.
# It reads its verbosity from these env vars ONCE, at import time -- so they MUST be
# set before `import decord` below, not after. Setting them later (e.g. alongside the
# ffmpeg av_log_set_level() call further down) has no effect, because decord's logger
# is already initialized by then.
...
print("[startup] decord native (dmlc-core) log level lowered to FATAL-only.")
...
try:
    ...
    print("[startup] ffmpeg log level lowered to FATAL.")
except Exception as e:
    print(f"[startup] could not lower ffmpeg log level ({e}); ffmpeg warnings may still print.")

# decord has its OWN logger (dmlc-core), separate from ffmpeg's -- av_log_set_level()
# above does NOT touch it, which is why "video_reader.cc:83: ERROR opening: ..." lines
# kept appearing even after ffmpeg was silenced. dmlc-core reads its verbosity from the
# DMLC_LOG_STACK_TRACE / glog-style env vars at import time, so this must be set BEFORE
# `import decord`. `import os` here explicitly rather than relying on the file's later
# `import os` (right before PYTORCH_CUDA_ALLOC_CONF) -- that import runs AFTER this
# block, and this block must run BEFORE `import decord`, which is even earlier still.
import os
os.environ.setdefault("GLOG_minloglevel", "3")     # 3 = FATAL only (dmlc-core uses glog conventions)
os.environ.setdefault("DMLC_LOG_STACK_TRACE", "0")
print("[startup] decord native (dmlc-core) log level lowered to FATAL-only.")
...
CKPT_PATHS = dict(
    # video_ff (FF++ Video Swin teacher) REINTRODUCED with a NEW checkpoint:
    # the old checkpoint measured near-random AUC (0.568) and was dropped;
    # this is a freshly trained torchvision Swin3D_T backbone (train_teacher_v6.py)
    # under a corrected face-crop + identity-aware-split pipeline, so it is
    # treated as a brand-new teacher, not a resumption of the old one.
    video_ff    = "/kaggle/input/models/syedazmulhasansabbir/ff-video-teacher-3/tensorflow2/default/1/best_teacher_model",
...
TEACHER_ORDER = ["video_ff", "frame_dfdc", "frame_diff", "audio_lavdf"]
TEACHER_BRANCH = {
    "video_ff": "temporal",
    "frame_dfdc": "frame",
    "frame_diff": "frame",
...
BRANCH_ORDER = ["frame", "temporal", "audio"]  # "temporal" stays: still a valid STUDENT branch
...
#
# video_ff is back in TEACHER_ORDER (see Section 0 above), now pointed at the
# NEW ff-video-teacher-3 checkpoint (train_teacher_v6.py, torchvision
# Swin3D_T backbone) rather than the old checkpoint that was dropped earlier.
# This is a DIFFERENT training configuration from whatever produced
# kd_no_video_ff's best_auc.pth/best_ema_auc.pth (those were trained WITHOUT
# video_ff's logit/temporal KD terms and a different domain prior), so
# RESUME_CKPT_PATH / BEST_CKPT_RESUME_INPUT_PATH / BEST_EMA_CKPT_RESUME_INPUT_PATH
# below are cleared. Run MODE='PRECOMPUTE_TEACHERS' to completion first, then
# 'TRAIN_STUDENT' to completion, then set BEST_CKPT_RESUME_INPUT_PATH /
# BEST_EMA_CKPT_RESUME_INPUT_PATH to point at THIS run's own checkpoints
# before switching to 'EVALUATE'.
MODE = "EVALUATE"

# RUN_TAG identifies this specific ablation/configuration. Every output path
# below (checkpoints, figures, CSVs, teacher artifacts) is namespaced under
# it, so this run's results can NEVER silently land in / overwrite the
# 4-teacher model's kd_final/ directory, and every artifact this run produces
# is traceable to exactly which teacher set produced it (see the
# ablation_config.json written by main(), Part 34).
RUN_TAG = "full_kd_ffvideo_v3" # 4-teacher run with the NEW ff-video-teacher-3 checkpoint

# ---- run control ----
OUT_DIR = Path(f"/kaggle/working/kd_{RUN_TAG}")
CKPT_DIR = OUT_DIR / "checkpoints"
# Cleared for this run: the previous ablation's checkpoints were trained
# WITHOUT video_ff, so KDTrainer's teacher_order guard would refuse to resume
# from them anyway now that video_ff is back in TEACHER_ORDER. Once
# MODE='TRAIN_STUDENT' has produced this run's own checkpoints under
# CKPT_DIR and you've saved that folder as a Kaggle dataset, point
# RESUME_CKPT_PATH / BEST_CKPT_RESUME_INPUT_PATH / BEST_EMA_CKPT_RESUME_INPUT_PATH
# at THOSE paths for any later session of THIS SAME run_tag.
RESUME_CKPT_PATH = "/kaggle/input/notebooks/hafsatalukder/ultimate-model-10/kd_full_kd_ffvideo_v3/checkpoints/last_checkpoint.pth"
FORCE_RESTART_CURRENT_EPOCH = True
BEST_CKPT_RESUME_INPUT_PATH = "/kaggle/input/notebooks/hafsatalukder/ultimate-model-10/kd_full_kd_ffvideo_v3/checkpoints/best_auc.pth"
BEST_EMA_CKPT_RESUME_INPUT_PATH = "/kaggle/input/notebooks/hafsatalukder/ultimate-model-10/kd_full_kd_ffvideo_v3/checkpoints/best_ema_auc.pth"
...
def csv_path(filename: str) -> Path:
    """Every CSV/JSON output name gets RUN_TAG (e.g. 'no_video_ff') stitched in,
    so a file copied out of OUT_DIR into a shared folder for the paper is
    self-describing -- 'domain_metrics_no_video_ff.csv' can never be mistaken
    for the full_kd run's 'domain_metrics_full_kd.csv' -- even if someone
    flattens both runs' csv/ directories together."""
    p = Path(filename)
    return CSV_DIR / f"{p.stem}_{RUN_TAG}{p.suffix}"

def fig_path(filename: str) -> Path:
    """Same rationale as csv_path(), for figures."""
    p = Path(filename)
    return FIG_DIR / f"{p.stem}_{RUN_TAG}{p.suffix}"

# ---- offline teacher-artifact store (Part 1/2/3/23) ----
ARTIFACT_DIR = OUT_DIR / "teacher_artifacts"
# Resuming the full_kd_ffvideo_v3 run after the kernel died mid-frame_diff
# (crashed at 20,544/328,596 samples). video_ff and frame_dfdc had already
# finished completely before the crash, and their shards/index/metadata are
# included in this same saved teacher_artifacts/ tree, so they'll be picked
# up as "0 remaining" too -- only frame_diff resumes mid-stream and
# audio_lavdf (which never started) runs from scratch this session.
ARTIFACT_RESUME_INPUT_DIR = (
    "/kaggle/input/notebooks/hafsatalukder/ultimate-model-10/"
    "kd_full_kd_ffvideo_v3/teacher_artifacts"
)

# No-op now that ARTIFACT_RESUME_INPUT_DIR is empty -- there is never a
# restored copy for _purge_force_regenerate_teachers() to act on. Left as an
# empty set (rather than removed) so pointing ARTIFACT_RESUME_INPUT_DIR at
# something again later doesn't also require remembering to re-add this.
ARTIFACT_FORCE_REGENERATE: Set[str] = set()
...
KD_TEMPERATURE = 4.0
# temporal=0.05 restored: video_ff is back in TEACHER_ORDER and is once again
# the only teacher that populates `seq` for temporal_kd_loss. hard drops back
# from 0.70 to 0.65 to give temporal_kd_loss its weight back rather than
# leaving it double-counted on top of `hard`.
...
# training) at all.
PRECOMPUTE_BATCH_SIZE_BY_BRANCH: Dict[str, int] = {"frame": 32, "temporal": 8, "audio": 16}
...
PRECOMPUTE_PREFETCH_BY_BRANCH: Dict[str, int] = {"frame": 2, "temporal": 2, "audio": 2}
...
RUN_EFFICIENCY_TABLE = False  # opt-in: param counts + inference latency, student vs. teachers
...
def resolve_ckpt_file(path: str) -> str:
    ...
    if not files:
        # Kaggle model versions sometimes ship a hidden ".data" bookkeeping folder
        # alongside the actual extracted-archive "data" folder -- exclude
        # dot-prefixed directories so a hidden ".data" sibling doesn't defeat the
        # single-subdirectory auto-descend below.
        ...
        if len(subdirs) == 1:
            print(f"[resolve_ckpt_file] {path} has no files at this level, only one "
                  f"non-hidden subdirectory ({subdirs[0].name}); descending into it "
                  f"(Kaggle nested the extracted archive one level deeper than usual, "
                  f"alongside a hidden bookkeeping folder).")
...
if TORCH_OK:
    ...
    TEACHER_EXPECTED = {
        "video_ff":    dict(modality="RGB video clip", input_shape="(B,3,16,224,224)",
    ...
    def _normalize_state_dict_keys(sd: dict, name: str) -> dict:
        # video_ff-specific "swin." -> "backbone." remap, restored: the new
        # ff-video-teacher-3 checkpoint was saved from a DeepfakeDetector
        # wrapper whose backbone attribute is named "swin" (train_teacher_v6.py),
        # while this file's VideoSwinFF class names the same submodule
        # "backbone" -- the checkpoint's keys must be remapped to match.
        out = {}
        for k, v in sd.items():
            nk = k
            ...
            if nk.startswith("module."):
                nk = nk[len("module."):]
            if name == "video_ff" and nk.startswith("swin."):
    ...
    def load_teacher_temperatures_from_artifacts(artifact_reader: "TeacherArtifactReader",
        ...
        save_json_static(csv_path("teacher_temperatures.json"), {k: v.T for k, v in scalers.items()})
        return scalers
    ...
    def build_default_domain_prior() -> Dict[str, Dict[str, float]]:
        ...
        frame_diff=0.796, audio_lavdf=0.896). video_ff is back with a NEW
        checkpoint (the old one's AUC=0.568 no longer applies) that has not
        been measured yet, so it starts at a neutral 1.0 -- same as
        frame_diff -- until an EVALUATE run's teacher_agreement.csv gives a
        real number to replace it with. "ffpp"-domain rows once again get a
        native-teacher boost on video_ff, same pattern as the other domains."""
        RELIABILITY = dict(frame_dfdc=0.7, frame_diff=1.0, audio_lavdf=1.6, video_ff=1.0)
    ...
    def mcnemar_test(labels, preds_a, preds_b) -> float:
        ...
        def _maybe_resume(self):
            ...
            if ckpt_teachers is not None and ckpt_teachers != list(TEACHER_ORDER):
                raise RuntimeError(
                    ...
                    f"silently mix two different KD configurations (e.g. fine-tuning a "
                    f"video_ff-trained student under a no-video_ff loss). Refusing to resume. "
                    f"Point RESUME_CKPT_PATH at a checkpoint from the SAME ablation, or clear it "
                    f"to start this configuration from scratch."
                )
        ...
        def run(self):
            ...
            while self.stage is not None:
                ...
                while self.epoch_in_stage < stage_epochs:
                    ...
                    pd_safe_write_csv(self.history, csv_path("training_history.csv"))
        ...
        def _run_epoch(self, loss_w: LossWeights, train: bool) -> dict:
            ...
            if train:
                ...
                if batch_weight_rows:
                    pd_safe_write_csv(batch_weight_rows, csv_path("teacher_weights.csv"),
                                       append=csv_path("teacher_weights.csv").exists())
            else:
                if all_labels:
    ...
    def _maybe_restore_artifact_dir_from_input():
        ...
        _purge_force_regenerate_teachers()


    def _purge_force_regenerate_teachers():
        """Discards any restored shards/index/metadata for teachers listed in
        ARTIFACT_FORCE_REGENERATE (currently {"video_ff"}), so
        precompute_teachers() sees zero completed samples for them and
        regenerates them from scratch this session -- while every other
        teacher's restored artifacts (frame_dfdc/frame_diff/audio_lavdf) are
        left completely untouched. This is what deletes the stale video_ff/
        shards copied in from ultimate-model-9's kd_no_video_ff run (they
        belong to the OLD video_ff checkpoint) without touching the reusable
        three-teacher artifacts alongside them."""
        if not ARTIFACT_FORCE_REGENERATE:
            return
        import shutil
        ...
        for name in ARTIFACT_FORCE_REGENERATE:
            shard_dir = ARTIFACT_DIR / name
            if shard_dir.exists():
                shutil.rmtree(shard_dir)
                print(f"[artifacts] force-regenerate: deleted restored {shard_dir} "
                      f"(stale -- will be recomputed from scratch this session).")
            if name in index:
                del index[name]
            if name in metadata:
                del metadata[name]
    ...
    def _maybe_restore_best_checkpoint_from_input():
        """/kaggle/working is wiped every fresh session, so the checkpoint from a
        prior TRAIN_STUDENT session must be copied in before EVALUATE can find it.
        Mirrors _maybe_restore_artifact_dir_from_input()'s pattern; also handles a
        Kaggle-extracted zip checkpoint via resolve_ckpt_file() (Part 26-adjacent).
        Restores BOTH best_auc.pth and best_ema_auc.pth when available (Part 35) --
        evaluate_ema_once() can produce a best_ema_auc.pth that beats best_auc.pth
        without self.best_auc (or best_auc.pth) ever being updated."""
        def _restore_one(input_path: str, dest_name: str):
            dest_path = CKPT_DIR / dest_name
            if dest_path.exists():
                return  # already present this session
            if not input_path:
                return
        ...
        if not (CKPT_DIR / "best_auc.pth").exists() and not (CKPT_DIR / "best_ema_auc.pth").exists():
            print("[checkpoint] neither BEST_CKPT_RESUME_INPUT_PATH nor "
                  ...
                  "EVALUATE will fail unless one is already present under "
                  "kd_no_video_ff/checkpoints/.")


    def _audit_and_drop_missing_files(rows: List[dict], branch: str, name: str) -> List[dict]:
        """Runs ONE cheap os.path.exists() pass over `rows` before any decord/torchaudio
        call is made for this teacher (Part 1-adjacent). If an entire upstream dataset
        input isn't mounted this session, EVERY row pointing at it fails identically --
        letting each of those rows reach _load_clip()/_load_audio() means: (a) decord's
        own native logger prints a line PER FILE that bypasses av_log_set_level()
        entirely (dmlc logging, not ffmpeg), (b) our own except-block prints a SECOND
        line per file, and (c) we still pay process/open overhead per attempt. None of
        that scales past a few dozen genuinely-corrupt files; at thousands of files
        from one unmounted dataset it just floods the log and hides real problems.
        This audit finds those rows up front, prints ONE aggregated summary per
        offending directory, and drops them from this session's todo list (they are
        NOT marked done in the artifact store -- re-running after fixing the mount
        will pick them up normally)."""
        path_key = {"frame": "frame_path", "temporal": "video_path", "audio": "audio_path"}[branch]
        present, missing = [], []
        for r in rows:
            p = r.get(path_key)
            if p is None or str(p).startswith("synthetic://") or Path(p).exists():
                present.append(r)
            else:
                missing.append(r)
        ...
        if missing:
            by_dir: Dict[str, int] = {}
            by_dir_example: Dict[str, str] = {}
            for r in missing:
                d = str(Path(r[path_key]).parent)
                by_dir[d] = by_dir.get(d, 0) + 1
                by_dir_example.setdefault(d, r[path_key])
            print(f"[precompute:{name}] WARNING: {len(missing)}/{len(rows)} rows' "
                  f"{path_key} do not exist on disk THIS SESSION -- skipping them for "
                  f"{name} without attempting decode (this usually means an input "
                  f"dataset isn't mounted, not a codec/decoder problem):")
            for d, count in sorted(by_dir.items(), key=lambda x: -x[1])[:10]:
                print(f"    {count:6d} missing under {d}  (e.g. {by_dir_example[d]})")
            if len(by_dir) > 10:
                print(f"    ... and {len(by_dir) - 10} more affected directories.")
            print(f"[precompute:{name}] these {len(missing)} rows remain in the manifest's "
                  f"'todo' set (not recorded as done) -- fix the missing input mount and "
                  f"re-run PRECOMPUTE_TEACHERS to pick them up.")
        return present
    ...
    def _precompute_one_teacher(name: str, todo_rows: List[dict], example_model: "nn.Module",
        ...
        default_bs = bs
        # Drop rows whose file is missing on disk BEFORE the retry/backoff loop ever
        # touches decord/torchaudio for them -- this is what stops the unbounded
        # per-file log spam at its source instead of just quieting it downstream.
        todo_rows = _audit_and_drop_missing_files(todo_rows, branch, name)
...
def precompute_teachers(all_rows: List[dict]):
    ...
    # Still run the full up-front coverage/forensics check (this only builds
    # throwaway models to verify checkpoints load cleanly) -- but do NOT keep
    # the resulting `teachers` dict. Holding all four teachers (video_ff +
    # frame_dfdc + frame_diff + audio_lavdf/WavLM, ~95M params) resident for
    # the WHOLE precompute session -- when only one is used per loop
    # iteration -- is what starved host RAM to ~2-3 GiB free by the time
    # frame_diff ran, causing the untrapped OOM-kill ("Kernel died") seen at
    # ~20,500 samples in BOTH the original run and the resumed run.
    _, load_reports = load_all_teachers(abort_on_error=True)
    ...
    for name in TEACHER_ORDER:
        ...
        # Load THIS teacher fresh, right before it's needed.
        example_model = TEACHER_CLASSES[name]()
        load_state_verified(example_model, CKPT_PATHS[name], name, abort_on_error=True)
        ...
        # Free this teacher's GPU + host memory before the next one loads.
        del example_model
        _free_gpu_memory(f"{name} unloaded after precompute")
    ...
    print("\n[precompute] all teachers processed for this session. "
...
def audit_manifest(rows: List[dict]) -> ManifestAuditReport:
    ...
    save_json_static(csv_path("manifest_audit.json"),
...
def _preflight_check_decoders(rows: List[dict], n_samples: int = 5):
    """Sampling here is spread across whatever datasets are present, NOT the
    first N manifest rows. Manifests are typically written grouped by dataset/
    source, so `rows[:n_samples]` can land entirely inside a single dataset.
    If THAT dataset's backing files happen to be missing from this particular
    session's mounted inputs (a dataset wasn't attached, or was swapped/renamed),
    every one of those N rows fails for a reason that has nothing to do with
    decord/torchaudio -- and the old code concluded the decoder itself was
    broken and aborted the whole run. Spreading the sample across datasets means
    one missing/unmounted dataset can only ever explain a PARTIAL failure, never
    a 0/n_samples wipeout, and a genuine decoder problem is never masked."""
    rng = random.Random(SEED)

    def _spread_sample(candidate_rows: List[dict], n: int) -> List[dict]:
        ...
        for ds_name in datasets:
            group = by_dataset[ds_name]
            k = min(per_dataset, len(group))
            out.extend(rng.sample(group, k))
        if len(out) < n:
            remaining = [r for r in candidate_rows if r not in out]
            if remaining:
                out.extend(rng.sample(remaining, min(n - len(out), len(remaining))))
        return out[:n]

    def _missing_on_disk(row: dict, path_key: str) -> bool:
        p = row.get(path_key)
        return bool(p) and not str(p).startswith("synthetic://") and not Path(p).exists()

    all_video_rows = [r for r in rows if _row_has_modality(r, "temporal")]
    all_audio_rows = [r for r in rows if _row_has_modality(r, "audio")]
    # Sample a few more than n_samples per-dataset-family so a single unmounted
    # dataset can't exhaust the whole sample; still capped at n_samples total below.
    video_rows = _spread_sample(all_video_rows, n_samples)
    audio_rows = _spread_sample(all_audio_rows, n_samples)
    ...
    if video_rows:
        ...
        missing = [r for r in video_rows if _missing_on_disk(r, "video_path")]
        ...
        if n_ok == 0:
            if missing:
                sample_missing = [r.get("video_path") for r in missing[:5]]
                missing_datasets = sorted({r.get("dataset", "unknown") for r in missing})
                raise RuntimeError(
                    f"[preflight] decord imported but ALL {len(video_rows)} sampled video files "
                    f"were not found on disk (dataset(s): {missing_datasets}), e.g. {sample_missing}. "
                    f"This looks like a missing/unmounted input dataset for this session, NOT a "
                    f"decord problem -- check that every dataset backing 'video_path' in the "
                    f"manifest is attached as a Kaggle input before re-running."
                )
            raise RuntimeError(f"[preflight] decord imported but ALL sample video decodes failed "
                                f"(files exist on disk; likely a real decode/codec problem).")
        print(f"[preflight] video decode OK: {n_ok}/{len(video_rows)} sample rows decoded "
              f"(sampled across {len({r.get('dataset','unknown') for r in video_rows})} dataset(s)).")
    ...
    if audio_rows:
        ...
        missing = [r for r in audio_rows if _missing_on_disk(r, "audio_path")]
        ...
        if n_ok == 0:
            if missing:
                sample_missing = [r.get("audio_path") for r in missing[:5]]
                missing_datasets = sorted({r.get("dataset", "unknown") for r in missing})
                raise RuntimeError(
                    f"[preflight] torchaudio imported but ALL {len(audio_rows)} sampled audio files "
                    f"were not found on disk (dataset(s): {missing_datasets}), e.g. {sample_missing}. "
                    f"This looks like a missing/unmounted input dataset for this session, NOT a "
                    f"torchaudio problem -- check that every dataset backing 'audio_path' in the "
                    f"manifest is attached as a Kaggle input before re-running."
                )
        ...
        print(f"[preflight] audio decode OK: {n_ok}/{len(audio_rows)} sample rows decoded "
              f"(sampled across {len({r.get('dataset','unknown') for r in audio_rows})} dataset(s)).")
...
if TORCH_OK:
    ...
    def build_data_loaders(rows: List[dict]):
        ...
        if len(calib_pool) > CALIB_POOL_MAX:
            rng = random.Random(SEED)
            calib_pool = rng.sample(calib_pool, CALIB_POOL_MAX)
        calib_loader = _loader(calib_pool, False)

        BASELINE_TRAIN_POOL_MAX = 8000
        baseline_train_pool = splits["train"]
        if len(baseline_train_pool) > BASELINE_TRAIN_POOL_MAX:
            rng = random.Random(SEED + 1)
            baseline_train_pool = rng.sample(baseline_train_pool, BASELINE_TRAIN_POOL_MAX)
        baseline_train_loader = _loader(baseline_train_pool, True)
        ...
        return splits["train"], val_loader, test_loader, calib_loader, baseline_train_loader, ood_loaders
    ...
    def build_stage_train_loader(all_train_rows: List[dict], stage: str) -> "DataLoader":
        ...
        log_path = csv_path("domain_class_sampling.csv")
    ...
    def _assert_checkpoint_matches_current_architecture(payload: dict, student: "nn.Module"):
        ...
        based) student checkpoint. Also refuses (Part 34) to evaluate a checkpoint
        trained under a DIFFERENT teacher set than the current TEACHER_ORDER --
        the student's weight SHAPE doesn't change when a teacher is dropped, so
        without this check a mismatched checkpoint would load "successfully" and
        silently produce numbers for the wrong ablation configuration."""
        ...
        if ckpt_teachers is not None and ckpt_teachers != list(TEACHER_ORDER):
            raise RuntimeError(
                ...
                f"produce numbers that don't match this run's ablation_config.json. Point "
    ...
    def _plot_all_figures(labels, probs, per_dataset: dict):
        ...
        def _save(fig, name):
            fig.savefig(fig_path(f"{name}.png"), dpi=150, bbox_inches="tight", facecolor="white")
            fig.savefig(fig_path(f"{name}.pdf"), bbox_inches="tight", facecolor="white")
            plt.close(fig)
    ...
    def run_comprehensive_evaluation_from_artifacts(student: "nn.Module",
        ...
        print("POOLED:", pooled)
        save_json(csv_path("pooled_metrics.json"), pooled)

        per_dataset = _grouped_metrics(labels, probs, all_datasets, all_ids)
        pd_safe_write_csv([dict(dataset=k, **v) for k, v in per_dataset.items()], csv_path("domain_metrics.csv"))

        per_manip = _grouped_metrics(labels, probs, all_manip, all_ids)
        pd_safe_write_csv([dict(manipulation=k, **v) for k, v in per_manip.items()], csv_path("error_analysis.csv"))

        per_regime = _grouped_metrics(labels, probs, all_regime, all_ids)
        pd_safe_write_csv([dict(regime=k, **v) for k, v in per_regime.items()], csv_path("threshold_analysis.csv"))
        ...
        pd_safe_write_csv(per_teacher_rows, csv_path("teacher_agreement.csv"))
        ...
        for ood_name, loader in ood_loaders.items():
            ...
            if ood_probs:
                ...
                pd_safe_write_csv([m], csv_path("cross_dataset_metrics.csv"), append=not first)
                first = False
                print(f"[OOD:{ood_name}]", m)
        ...
        return dict(pooled=pooled, per_dataset=per_dataset, per_manip=per_manip, per_regime=per_regime,
                    raw=dict(ids=all_ids, labels=labels, probs=probs))

    def train_and_eval_baselines_from_artifacts(artifact_reader: "TeacherArtifactReader",
                                                 usable: Dict[str, bool],
                                                 scalers: Dict[str, TemperatureScaler],
                                                 train_loader: "DataLoader", val_loader: "DataLoader",
                                                 test_loader: "DataLoader") -> Tuple[List[dict], Dict[str, dict]]:
        ...
        for n in embed_dims:
            ...
            if m["auc"] == m["auc"] and m["auc"] > best_single["auc"]:
                best_single = dict(name=n, auc=m["auc"])
        ...
        pd_safe_write_csv(rows, csv_path("baseline_comparison.csv"))
    ...
    def run_significance_tests(student_raw: dict, baseline_raw: Dict[str, dict]) -> List[dict]:
        ...
        pd_safe_write_csv(rows, csv_path("significance_tests.csv"))
    ...
    def _plot_auc_comparison_with_ci(labels: List[str], aucs: List[float],
        ...
        fig.savefig(fig_path("auc_comparison_ci.png"), dpi=150, bbox_inches="tight", facecolor="white")
        fig.savefig(fig_path("auc_comparison_ci.pdf"), bbox_inches="tight", facecolor="white")
        plt.close(fig)
        print(f"[significance] wrote auc_comparison_ci_{RUN_TAG}.png/pdf to {FIG_DIR}")   
    ...
    def run_efficiency_comparison(student: "nn.Module") -> List[dict]:
        ...
        def _measure(name: str, model: "nn.Module", make_input, n_runs: int = 30):
        ...
        _measure("frame_diff_teacher", StudentEfficientNetB2(),
                  lambda: torch.randn(8, 3, 224, 224, device=DEVICE))
        # video_ff_teacher removed: that teacher is dropped from the pipeline.
        ...
        rows.append(dict(model="all_3_teachers_combined (HGF-baseline inference cost)",
                          ...
                          device=str(DEVICE)))
        pd_safe_write_csv(rows, csv_path("efficiency_comparison.csv"))
        return rows 
    # ══════════════════════════════════════════════════════════════════════
    # 16. MAIN ORCHESTRATION — mode-separated (Part 16/21/29)
    # ══════════════════════════════════════════════════════════════════════
    def write_ablation_config():
        """Part 34: every EVALUATE-mode CSV/figure this run produces should be
        traceable to exactly which teacher set and loss weights produced it --
        otherwise a leave-one-teacher-out ablation's numbers are indistinguishable
        from the full-KD run's the moment they're pulled out of OUT_DIR for the
        paper. Written unconditionally at the top of main(), for all three MODEs,
        so even a TRAIN_STUDENT-only or PRECOMPUTE_TEACHERS-only session leaves a
        record of the config it ran under."""
        config = dict(
            run_tag=RUN_TAG,
            mode=MODE,
            teacher_order=list(TEACHER_ORDER),
            teacher_branch=dict(TEACHER_BRANCH),
            loss_weights=dict(LOSS_WEIGHTS),
            kd_temperature=KD_TEMPERATURE,
            stage_epochs=dict(STAGE_EPOCHS_V2),
            stage_batch_size=dict(STAGE_BATCH_SIZE_V2),
            active_branches_by_stage={k: sorted(v) for k, v in ACTIVE_BRANCHES_BY_STAGE_V2.items()},
            domain_prior=build_default_domain_prior(),
            seed=SEED,
        ...
        out_path = csv_path("ablation_config.json")
    ...
    def main():
        ...
        train_rows, val_loader, test_loader, calib_loader, baseline_train_loader, ood_loaders = \
        ...
        if MODE == "TRAIN_STUDENT":
            ...
            if trainer.stage is None:
                ...
                if not run_feasibility_gate(trainer, timing):
                    ...
                    return
            trainer.run()
            router.flush_weight_log(csv_path("teacher_router_weights_by_domain_epoch.csv"))
            if trainer.stage is None:
                print("\n[main] DONE (TRAIN_STUDENT) -- ALL STAGES COMPLETE. Checkpoints under:",
                      CKPT_DIR,
                      "\n[main] Switch MODE to 'EVALUATE' in a dedicated session to run the full "
                      "test/OOD/baseline evaluation suite against whichever of best_auc.pth / "
                      "best_ema_auc.pth records the higher val AUC.")
            else:
                print(f"\n[main] SESSION ENDED EARLY (budget) -- still in stage={trainer.stage} "
                      f"({STAGE_NAMES_V2.get(trainer.stage)}), "
                      f"epoch_in_stage={trainer.epoch_in_stage}/{STAGE_EPOCHS_V2[trainer.stage]}. "
                      f"Checkpoints under: {CKPT_DIR}."
                      "\n[main] Do NOT switch to 'EVALUATE' yet. Re-run MODE='TRAIN_STUDENT' "
                      "in a new session with RESUME_CKPT_PATH pointed at this session's saved "
                      "last_checkpoint.pth to continue training.")
            return
        ...
        if MODE == "EVALUATE":
            ...
            # Part 35: self.best_auc is only ever updated by per-epoch val passes, never
            # by the one-time end-of-run EMA eval -- so best_ema_auc.pth can (and here
            # did: 0.9362 vs best_auc.pth's 0.9324) record a higher AUC than best_auc.pth
            # without best_auc.pth reflecting that at all. Load whichever file's recorded
            # AUC is actually higher instead of hardcoding best_auc.pth.
            candidates = []
            ...
            baseline_rows, baseline_raw = train_and_eval_baselines_from_artifacts(
                artifact_reader, usable, scalers, baseline_train_loader, val_loader, test_loader)
            run_significance_tests(results["raw"], baseline_raw)
            ...
            if RUN_EFFICIENCY_TABLE:
                run_efficiency_comparison(student)
            else:
